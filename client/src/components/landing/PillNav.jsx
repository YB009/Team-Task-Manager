...
const PillNav = ({
  ...
  const handleSectionScroll = (href, closeMobile = false) => (event) => {
      ...
      >
        <ul className="list-none m-0 p-[3px] flex flex-col gap-[3px]">
          {items.map((item) => {
            ...
            const linkClasses = "block py-3 px-4 text-[14px] font-medium rounded-[50px] transition-all duration-200";
              ...
              <li key={item.href}>
                {isRouterLink(item.href) ? (
                  ...
                  <a
                    ...
                    className={linkClasses}
                    style={defaultStyle}
                    onClick={handleSectionScroll(item.href, true)}
                    onMouseEnter={hoverIn}
                    onMouseLeave={hoverOut}
                  >
                    {item.label}
